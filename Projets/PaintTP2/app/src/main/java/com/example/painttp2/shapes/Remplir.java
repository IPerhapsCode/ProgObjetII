...
public class Remplir extends Formes{
    ...
    public void fill(int x, int y)
    {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q)
        {
            if(x >= 0 && x < this.bitmap.getWidth()
                    && y >= 0 && y < this.bitmap.getHeight())
            {
                this.currentColor = this.bitmap.getColor(x, y).toArgb();

                if(this.initialColor == this.currentColor)
                {
                    if (MainActivity.couleurs.contains(this.color))
                    {
                        this.bitmap.setPixel(x, y, ContextCompat.getColor(this.context, this.color));
                    }
                    else
                    {
                        this.bitmap.setPixel(x, y, this.color);
                    }

                    if(x + 1 < this.bitmap.getWidth())
                    {
                        fill(x + 1, y);
                    }
                    if(x - 1 >= 0)
...
//            HashMap<Point, Integer[]> pixels = new HashMap<>(10000, 10000);
...
////                        for(Point k : j.keySet())
////                        {
////                            pixels.put(k, new Integer[2]);
////                            pixels.get(k)[0] = myGroup.size();
////                            pixels.get(k)[1] = i;
////                        }
//                        breakTime = true;
//                    }
